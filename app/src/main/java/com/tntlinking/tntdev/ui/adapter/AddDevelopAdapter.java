...
public final class AddDevelopAdapter extends AppAdapter<ExperienceBean> {

    public static final int ADD_EDUCATION_TAB = 1;
    public static final int ADD_WORK_TAB = 2;
    public static final int ADD_PROJECT_TAB = 3;
    public static final int ADD_EDUCATION = 4;
    public static final int ADD_WORK = 5;
    public static final int ADD_PROJECT = 6;

    private Context mContext;

    public AddDevelopAdapter(Context context) {
...
//            mTitleView.setText(getItem(position).getStr());
    ...
    private final class AddWorkViewHolder extends ViewHolder {

        private final TextView tv_work_name;
        private final TextView tv_work_position;
        private final TextView tv_work_time;

        private AddWorkViewHolder() {
            super(R.layout.item_tab_work);
            tv_work_name = findViewById(R.id.tv_work_name);
            tv_work_position = findViewById(R.id.tv_work_position);
        ...
        public void onBindView(int position) {
            ExperienceBean item = getItem(position);
            tv_work_name.setText(item.getCollegeName());
            String time = item.getInSchoolStartTime() + " - " + item.getInSchoolEndTime();
            tv_work_position.setText(time);
            String info = item.getEducation() + " | " + item.getmTrainingMode() + " | " + item.getMajor();
            tv_work_time.setText(info);
